#!/usr/bin/env python3
"""
MuJoCo sim2sim deployment for LeggedLab-trained G1 policies (BeyondMimic PD gains).

Loads an ActorCriticRecurrent (LSTM) or ActorCritic (MLP) checkpoint and
deploys it in MuJoCo with BeyondMimic PD parameters.

This variant uses frequency-based stiffness/damping derived from motor armature,
matching the BeyondMimic configuration in legged_lab/assets/unitree/unitree.py
(STIFFNESS_5020, STIFFNESS_7520_14, etc.).

Key differences from deploy_mujoco.py (original):
  - PD gains:  frequency-based (armature * omega_n^2), much lower arm stiffness
                (shoulder ~14 vs 100, elbow ~14 vs 50 Nm/rad)
  - Default joint angles: deeper crouch, different arm pose
  - Effort limits: feet=50, waist=50 (vs original 35, 35)
  - Actuator grouping: waist split into waist + waist_yaw, arms cover all upper body

Usage:
  cd AME_mujoco_sim2sim
  python deploy_mujoco_beyondmimic.py --policy_path ../logs/g1_rough/.../model_15700.pt
"""
import time
import os
import sys
...
# =============================================================================
# BeyondMimic motor parameters (from legged_lab/assets/unitree/unitree.py)
# =============================================================================
# Stiffness = armature * (2*pi*f)^2,  Damping = 2*zeta*sqrt(armature*stiffness)
# With f=10Hz, zeta=2.0 this gives critically-damped position PD controllers.
...
NATURAL_FREQ = 10.0 * 2.0 * math.pi         # 62.83 rad/s
DAMPING_RATIO = 2.0
...
# Default joint angles in POLICY_JOINT_ORDER, from BeyondMimic G1_CYLINDER_CFG.init_state.joint_pos.
# These are the regex patterns used in unitree.py (BeyondMimic config).
# Deeper crouch (hip_pitch=-0.312, knee=0.669) and more compact arm pose vs original.
...
# =============================================================================
# PD gains from BeyondMimic G1_CYLINDER_CFG actuators (unitree.py)
# =============================================================================
# Stiffness = armature * omega_n^2, Damping = 2*zeta*sqrt(armature*stiffness)
# where omega_n = 2*pi*10 = 62.83 rad/s, zeta = 2.0
#
# Group overview:
#   legs:      STIFFNESS_7520_14/22, DAMPING_7520_14/22  (hip_pitch=40, knee=99)
#   feet:      2*STIFFNESS_5020,     2*DAMPING_5020       (~28.5, ~1.81)
#   waist:     2*STIFFNESS_5020,     2*DAMPING_5020       (~28.5, ~1.81)
#   waist_yaw: STIFFNESS_7520_14,    DAMPING_7520_14      (~40.2, ~2.56)
#   arms:      STIFFNESS_5020/4010,  DAMPING_5020/4010    (~14.3/16.8, ~0.91/1.07)
# =============================================================================
