...
The policy input mirrors ``LeggedLab-Isaac-AMP-Rough-G1-Play-v0``:
    ...
    xyz terrain map           1 x 11 x 11 x 3
...
class TrainingCheckpointPolicy:
    ...
    def __init__(
        ...
        num_actions: int,
        map_scan_dim: tuple[int, int, int],
        map_scan_history_length: int,
    ) -> None:
        self.device = device
        self.expected_obs_dim = expected_obs_dim
        self.num_actions = num_actions
        ...
        map_scan_size = int(np.prod(map_scan_dim)) * map_scan_history_length
        critic_obs_dim = self._infer_critic_obs_dim(checkpoint, map_scan_size, expected_obs_dim)

        try:
            from rsl_rl.modules import ActorCriticEncoder
        except ImportError as error:
            raise RuntimeError(
                "Loading a training checkpoint requires rsl_rl.modules.ActorCriticEncoder in the active Python "
                "environment. Use an exported TorchScript policy.pt or activate the Isaac Lab environment."
            ) from error

        self.module = ActorCriticEncoder(
            ...
            map_scan_history_length=map_scan_history_length,
            mha_dim=64,
            num_heads=16,
    ...
    @staticmethod
    def _infer_critic_obs_dim(checkpoint: dict[str, Any], map_scan_size: int, fallback: int) -> int:
        critic_state_dict = checkpoint.get("critic_state_dict")
        if isinstance(critic_state_dict, dict):
            proprio_weight = critic_state_dict.get("proprio_embedding.weight")
            if isinstance(proprio_weight, torch.Tensor) and proprio_weight.ndim == 2:
                return int(proprio_weight.shape[1]) + map_scan_size
        return fallback
...
def main() -> None:
    ...
    if (scanner.num_x, scanner.num_y, 3) != (11, 11, 3):
        raise ValueError(
            "The current AME actor requires an 11x11x3 map, "
            f"but the deployment config produces {scanner.num_x}x{scanner.num_y}x3."
        )
    ...
    if expected_obs_dim != 858:
        raise ValueError(f"The current G1 AMP rough actor requires 858 observations, got {expected_obs_dim}.")
    ...
    policy = load_policy(
        ...
        expected_obs_dim,
        num_actions,
        (scanner.num_x, scanner.num_y, 3),
        height_history_length,
    )
    ...
    print(f"  Policy input:    {expected_obs_dim} = 495 proprio history + 363 terrain")
    print(f"  Terrain map:     {scanner.num_x} x {scanner.num_y} x 3 @ {scan_resolution:.3f} m")
