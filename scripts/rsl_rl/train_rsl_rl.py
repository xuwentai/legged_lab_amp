...
Called by ``./isaaclab.sh train --rl_library rsl_rl --task <TASK>``.
``common.py`` (from ``IsaacLab/scripts/reinforcement_learning/``) is added to
PYTHONPATH automatically by the isaaclab.sh launcher.
"""
...
# Import legged_lab tasks (pxr-free after TYPE_CHECKING refactor)
...
def run(argv: list[str]) -> None:
    """Train an RSL-RL agent (called by ``./isaaclab.sh train --rl_library rsl_rl``)."""
    import torch
    ...
    # resolve_task_config is safe to call before Kit is launched (uses ResolvableString)
