"""AME terrain-attention model for the RSL-RL 5.x split actor/critic API."""
...
class AMEEncoder(nn.Module):
    """Encode a trailing terrain map with a CNN and proprioceptive cross-attention.

    The selected observation groups are concatenated in their configured order. The
    final ``history * length * width * coord_dim`` values must contain the flattened
    terrain scan; all preceding values are treated as proprioception.
    """
    ...
    def __init__(
        ...
        map_scan_dim: tuple[int, int, int] | list[int] = (16, 11, 3),
        ...
        if len(map_scan_dim) != 3:
            raise ValueError(f"map_scan_dim must contain (length, width, coord_dim), got {map_scan_dim}.")
        if map_scan_history_length < 1:
            raise ValueError(f"map_scan_history_length must be at least 1, got {map_scan_history_length}.")
        ...
        self.map_scan_history_length = map_scan_history_length
        self.map_scan_size = self.length * self.width * self.coord_dim * map_scan_history_length
        self.proprio_dim = self.obs_dim - self.map_scan_size
        if self.proprio_dim <= 0:
            raise ValueError(
                f"AMEEncoder expects obs_set '{obs_set}' to end with a flattened terrain map of "
                f"{self.map_scan_size} values, but its total observation dimension is {self.obs_dim}."
            )
        ...
        head_input_dim = self.proprio_dim + self.mha_dim * (2 if attach_global else 1)
    ...
    def get_latent(
        ...
        encoded, _ = self._encode_terrain(flat_obs)
        return encoded

    def get_latent_with_attention(self, obs: TensorDict) -> tuple[torch.Tensor, torch.Tensor]:
        """Return the encoded latent and spatial attention weights for visualization."""
        flat_obs = torch.cat([obs[obs_group] for obs_group in self.obs_groups], dim=-1)
    ...
    def as_jit(self) -> nn.Module:
        return _TorchAMEEncoder(self)

    def as_onnx(self, verbose: bool) -> nn.Module:
        return _OnnxAMEEncoder(self, verbose)

    def _build_terrain_encoder(self) -> None:
        input_channels = self.coord_dim * self.map_scan_history_length
    ...
    def _encode_terrain(self, flat_obs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if flat_obs.shape[-1] != self.obs_dim:
            raise ValueError(f"Expected flat observation dimension {self.obs_dim}, got {flat_obs.shape[-1]}.")

        proprio_obs = self.obs_normalizer(flat_obs[..., : self.proprio_dim])
        map_scan = flat_obs[..., self.proprio_dim :].reshape(
            -1,
            self.map_scan_history_length,
            self.width,
            self.length,
            self.coord_dim,
        )
        map_features = map_scan.permute(0, 4, 1, 2, 3).reshape(
            -1,
            self.coord_dim * self.map_scan_history_length,
            self.width,
            self.length,
        )
    ...
    def _get_obs_dim(
        ...
        for obs_group in active_obs_groups:
            if len(obs[obs_group].shape) != 2:
                raise ValueError(
                    f"AMEEncoder only supports flattened observations, got shape "
                    f"{obs[obs_group].shape} for group '{obs_group}'."
                )
            obs_dim += obs[obs_group].shape[-1]
        return active_obs_groups, obs_dim


class _TorchAMEEncoder(nn.Module):
    """Tensor-only deterministic wrapper used by JIT and ONNX export."""

    def __init__(self, model: AMEEncoder) -> None:
        super().__init__()
        self.proprio_dim = model.proprio_dim
        self.length = model.length
        self.width = model.width
        self.coord_dim = model.coord_dim
        self.map_scan_history_length = model.map_scan_history_length
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        proprio_obs = self.obs_normalizer(obs[..., : self.proprio_dim])
        map_scan = obs[..., self.proprio_dim :].reshape(
            -1,
            self.map_scan_history_length,
            self.width,
            self.length,
        ...
        cnn_features = self.map_cnn(map_features)
        local_features = cnn_features.permute(0, 2, 3, 1).reshape(obs.shape[0], -1, self.mha_dim)
        ...
        if global_features_max is not None:
            encoded = torch.cat([global_features_max, encoded], dim=-1)
        return self.deterministic_output(self.mlp(encoded))
...
class _OnnxAMEEncoder(_TorchAMEEncoder):
    is_recurrent: bool = False

    def __init__(self, model: AMEEncoder, verbose: bool) -> None:
